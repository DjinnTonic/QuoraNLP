...
def main():
    ...
    print("Forming questions: " + str(ques.shape[0]))
    for i in range(ques.shape[0]):
        q_dict[ques.question1[i]].add(ques.question2[i])
        q_dict[ques.question2[i]].add(ques.question1[i])
        if (i % (ques.shape[0] // 100)) == 0:
            print(str(i // (ques.shape[0]//100)) + "%")
    ...
    x_test_ab = df_test.iloc[:, 2:-1] ####### Is this eliminating the csv-comma error?
